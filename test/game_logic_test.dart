...
import 'package:star_trails/data/wisdom_entries.dart';
...
void main() {
  ...
  group('Versioned Save Reset Policy', () {
    ...
    test('Changing release line clears saved dashboard/device-linked data',
        () async {
  ...
  group('Game flow (provider commands)', () {
    test('startNewGame clears wisdom and calculator history', () async {
      SharedPreferences.setMockInitialValues({});

      final provider = GameProvider();
      await provider.dashboard.initialize();
      await provider.dashboard.startSession();
      provider.updateSessionId(provider.dashboard.getCurrentSessionId());

      provider.wisdomEngine
          .markWisdomShown(grade4to6Wisdom.first, savedToLogbook: true);
      await provider.dashboard.addWisdomEntry(
        text: grade4to6Wisdom.first.text,
        gradeTier: '4-6',
      );
      ...
      expect(provider.wisdomEngine.sessionWisdom, isNotEmpty);
      expect(provider.dashboard.getWisdomEntries(), isNotEmpty);
      expect(provider.dashboard.getTradingCalculations(), isNotEmpty);

      await provider.startNewGame();

      expect(provider.wisdomEngine.sessionWisdom, isEmpty);
      expect(provider.dashboard.getWisdomEntries(), isEmpty);
      expect(provider.dashboard.getTradingCalculations(), isEmpty);
      expect(provider.state.currentSessionId, isNotEmpty);
    });
  ...
  group('Achievement Unlock Display', () {
    ...
    test('tier 1 systems become active after reaching threshold', () {
      final state = GameState.initial();
      final updatedTierStates =
          Map<String, CRAccessState>.from(state.tierAccessStates);
      updatedTierStates['1'] =
          const CRAccessState(discovered: true, accessActive: true);
    ...
    test('tier 1 discovered status persists independently of access active',
      ...
      updatedTierStates['1'] =
          const CRAccessState(discovered: true, accessActive: false);
      final newState = state.copyWith(tierAccessStates: updatedTierStates);
    ...
    test('commodities remain unlocked even when system access is suspended',
      ...
      expect(
        ...
        isTrue,
        reason:
            'Tech commodity should remain unlocked even when system access is suspended',
      );
    });
    ...
    test('hysteresis prevents flickering when credits hover near threshold',
      ...
      expect(isActive, isTrue,
          reason: 'Should stay active above lock threshold');
      ...
      expect(isActive, isFalse,
          reason: 'Should become inactive below lock threshold');
      ...
      expect(isActive, isFalse,
          reason: 'Should not reactivate until reaching 80% threshold');
